...
for (i,PLATFORM) in enumerate(PLATFORMs), (j,d) in enumerate(ds), (k,N) in enumerate(Ns), (l,crop_tol) in enumerate(crop_tols) # hide
    ...
    M = plunge*A; # hide
    colsizes[i,j,k,l], rowsizes[i,j,k,l]  = truncated_size(ReductionSolver(M; crop_tol=crop_tol)) # hide
end # hide
...
for (i,PLATFORM) in enumerate(PLATFORMs), (j,d) in enumerate(ds), (k,N) in enumerate(Ns) #hide
    P = ExtensionFramePlatform(PLATFORM(d), 0.0..0.5) #hide
    F,_ = @timed Fun(exp, P, N;REG=BSplineExtension.ReductionSolver, crop=true, crop_tol=1e-10) #hide
    errors[i,j,k] = abserror(exp, F) #hide
end #hide
for (i,PLATFORM) in enumerate(PLATFORMs), (j,d) in enumerate(ds), (k,N) in enumerate(Ns1) #hide
    P = ExtensionFramePlatform(PLATFORM(d), 0.0..0.5); #hide
    _,timings[i,j,k],_ = @timed Fun(exp, P, N;REG=BSplineExtension.ReductionSolver,  crop=true, crop_tol=1e-10) #hide
end # hide
A = [];for (i,PLATFORM) in enumerate(PLATFORMs) # hide
...
#     colsizes[i,j,k,l], rowsizes[i,j,k,l]  = truncated_size(ReductionSolver(M; crop_tol=crop_tol,lazy=true)) # hide
# end # hide
# A = [];for (i,PLATFORM) in enumerate(PLATFORMs) # hide
...
# for (i,PLATFORM) in enumerate(PLATFORMs), (j,d) in enumerate(ds), (k,N) in enumerate(Ns1) #hide
#     P = ExtensionFramePlatform(PLATFORM((d,d)), (0.0..0.5)^2) #hide
#     F,_ = @timed Fun(f, P, (N,N);REG=BSplineExtension.ReductionSolver, crop=true, crop_tol=1e-10) #hide
#     errors[i,j,k] = abserror(f, F) #hide
# end #hide
# for (i,PLATFORM) in enumerate(PLATFORMs), (j,d) in enumerate(ds), (k,N) in enumerate(Ns2) #hide
#     P = ExtensionFramePlatform(PLATFORM((d,d)), (0.0..0.5)^2) #hide
#     _,timings[i,j,k],_ = @timed Fun(f, P, (N,N);REG=BSplineExtension.ReductionSolver, crop=true, crop_tol=1e-10) #hide
# end # hide
# A = [];for (i,PLATFORM) in enumerate(PLATFORMs) # hide
